...
number known to be divisible by 7 is obtained; you can stop when this number has at most 2 digits because you
are supposed to know if a number of at most 2 digits is divisible by 7 or not.
...
fun seven(n:Long):LongArray {
    ...
    var number = n
    while (number > 99) {
        number = (number-(number % 10))/10 - ((number % 10)*2)
        counter++
    }
        return longArrayOf(number, counter)
    }

fun seven2(n: Long, i: Long = 0): LongArray =
    if (n > 99) seven2(n / 10 - n % 10 * 2, i + 1)
    else longArrayOf(n, i)

fun main(){
  println(seven(477557101).joinToString())
}
